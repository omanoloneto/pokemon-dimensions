# Pokémon Dimensions — RPG Maker MZ

Jogo estilo Pokémon multi-franquia em **RPG Maker MZ** (JavaScript), reescrito a
partir do projeto original em Pokémon Essentials / RPG Maker XP (Ruby). A raiz
deste repositório É o projeto MZ (abra `game.rmmzproject` no editor).
...
```bash
node tools/test_harness.js   # todos os sistemas (56 testes)
ruby tools/compile_all.rb    # recompila os data/*.json do PBS
ruby tools/migrate_xp.rb     # exporta o conteúdo dos mapas do projeto XP
```
